...
**None of this is required** — My Bookshelf works fully as a local-only
localStorage/IndexedDB app without it.

## What it does

- Backs up all your books to a Supabase project you own: one Postgres
  table (`backup_records`) plus two Edge Functions (`backup-sync` for the
  data, `backup-image` for cover photo bytes), all gated behind a
  passphrase only your own devices know.
- Cover photo uploads go to the project's own private Storage bucket
  instead of being inlined in the synced record — a device only downloads
  a given cover the first time it's actually displayed (a lazy-loaded
  book card scrolling into view, say), then caches it locally so every
...
My Bookshelf (and each other Make It Local app with Cloud Sync) needs its
own registered Supabase OAuth application so "Connect Supabase" can act on
your behalf via the Management API. This step can't be automated — it
requires your own Supabase login — and has to be done once per app you
deploy, since the redirect URI is tied to that app's own domain.

1. Go to your Supabase organization's **OAuth Apps** settings
   (`https://supabase.com/dashboard/org/_/apps` — pick your org first) →
   **Add application**.
2. Name it anything (e.g. "My Bookshelf Cloud Sync") and set its redirect
   URI to exactly `https://<your deployed domain>/api/oauth-callback`.
...
- `backup_records`' `store` column has no database-level restriction on
  ...
  `SYNCABLE_STORES`), so multiple apps' data (this app's `books`, a
  sibling's `cards`, another's `profiles`/`tags`/`snippets`) coexists
