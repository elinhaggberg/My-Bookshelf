// Storage-based image sync for Cloud Backup -- uploads a book's actual
// cover-photo bytes once to this project's private Supabase Storage bucket
// via the backup-image Edge Function, and syncs a small
...
// periodic sync) never has to download every cover up front, only the ones
...
// nothing about it changed. Not a content hash: a save is the only moment a
// cover could have changed, and saveBook already bumps updatedAt on every
// save, so that's a good enough signal at personal-register scale.
const SYNCED_VERSIONS_KEY = "mb_image_synced_versions_v1";
...
// "idb:<id>" cover is uploaded (once per change, see alreadySynced above)
...
// remote unfurled cover URL, or no cover at all) passes through unchanged.
//
...
// cover is dropped from the outgoing record (never the raw idb: reference,
...
export async function syncRecordImageForPush(store, record) {
  ...
  if (alreadySynced(recordId, updatedAt)) return { ...record, coverImage: ref };

  const config = getApiConfig();
  const passphrase = getBackupPassphrase();
  if (!config?.url || !config?.anonKey || !passphrase) return { ...record, coverImage: null };

  const blob = await getImage(recordId);
  if (!blob) return { ...record, coverImage: null };
  ...
  try {
    ...
    if (!res.ok) return { ...record, coverImage: null };
    markSynced(recordId, updatedAt);
    return { ...record, coverImage: ref };
  } catch {
...
// so every existing resolveImageSrc call site (book covers, search
// results...) picks up storage: references automatically, no call-site
// changes needed. Fetches a short-lived signed URL from the Edge Function,
// downloads the bytes once, and caches them into this device's own image
// store -- patchRecordImage (injected per app, since each app's records
// live in a differently-shaped local store) then rewrites the record's
// image field from the storage: reference to a plain idb: one, so every
// subsequent open resolves instantly with no network round trip at all,
// exactly like a record whose cover was always local to begin with.
