...
// actual books to the connected Supabase project. Same "bring your own
// database" principle as the rest of Cloud Sync: nothing here works until
// someone connects a project and installs Cloud Backup specifically.
//
...
const PASSPHRASE_KEY = "mb_backup_passphrase_v1";
const LAST_SYNCED_KEY = "mb_backup_last_synced_v1";
const PERIODIC_INTERVAL_MS = 15 * 60 * 1000;
const SYNCABLE_STORES = ["books"];
// A single non-content record (theme/homeTitle, see storage.js's
...
// syncRecordImageForPush uploads a device-local idb: cover to Storage and
// swaps in a portable storage: reference (or leaves a remote URL/no-cover
// record untouched) -- see cloudImageSync.js's own comment on it. Without
// this, a synced record's cover would just be a meaningless local
...
export async function pushAll({ getBooks, getTombstones, clearTombstones, getPrefsSnapshot, getPrefsUpdatedAt }) {
  if (!isBackupConfigured()) return { applied: 0 };
  const [books, tombstones] = await Promise.all([getBooks(), getTombstones()]);
  const records = [
    ...(await Promise.all(books.map((b) => toRecord("books", b)))),
  ...
  // deletion. Each tombstone's uploaded cover (if it had one) is cleaned up
